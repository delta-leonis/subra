 ...
 *
 * Reads {@link org.robocup.ssl.RefereeOuterClass.Referee} and formats this into a {@link org.robocup.ssl.RefereeOuterClass.Referee}
 *
 * @author Jeroen de Jong
 */
@AllArgsConstructor
public class SSLRefboxDeducer implements Deducer<org.robocup.ssl.RefereeOuterClass.Referee, Referee> {
  @Override
  public Publisher<Referee> apply(final Publisher<org.robocup.ssl.RefereeOuterClass.Referee> refboxPublisher) {
    return Flux.from(refboxPublisher)
        .map(packet -> new Referee.State(
            ...
            ImmutableSet.of(
                this.createTeam(TeamColor.BLUE, packet.getBlue()),
                this.createTeam(TeamColor.YELLOW, packet.getYellow())),
            packet.getCommandTimestamp(),
            packet.getCommandCounter()));
  }

  private Team createTeam(final TeamColor teamColor, final TeamInfo teamInfo) {
